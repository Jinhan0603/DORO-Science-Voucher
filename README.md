# DORO Science Voucher Education Portal

🔬 **DORO 과학문화바우처 교육 포털** — AI × IoT 융합 STEAM 교육

키트를 구매한 사용자가 해당 링크에 접속하면 구매 키트에 맞춰 단계별 교육 자료를 바로 확인할 수 있는 교육 포털입니다.

## 🌐 사이트 바로가기

| 버전 | 링크 | 설명 |
|------|------|------|
| **V3** ✨ 운영 중 | [DOROLAND V3 →](https://jinhan0603.github.io/DORO-Science-Voucher/) | 기존 운영 버전 |
| **V4** 🆕 미리보기 | [DORO V4 Guide →](https://jinhan0603.github.io/DORO-Science-Voucher/v4/) | QR 기반 키트 가이드 · 구성품 확인 · 안전 약속 · 단계별 조립 |

루트 `/`는 기존 V3를 유지하고, `/v4/`에서 새 가이드를 별도로 확인할 수 있습니다.

## 🚀 프로그램 목록

| # | 프로그램 | 하드웨어 | AI 도구 | 시간 |
|---|---------|---------|---------|------|
| 1 | 블루투스 스피커 × Suno 음악 체험 | IoT 블루투스 스피커 | [Suno](https://suno.com) | 90분 |
| 2 | 무드등 × ChatGPT 스토리 꾸미기 | 조도센서 + LED 무드등 | [ChatGPT](https://chat.openai.com) | 90분 |
| 3 | 워킹 로봇 × Vrew 영상 편집 | 이족보행 로봇 | [Vrew](https://vrew.voyagerx.com) | 120분 |
| 4 | IR 자동차 × Teachable Machine | IR 장애물 인식 자동차 | [Teachable Machine](https://teachablemachine.withgoogle.com/) | 120분 |
| 5 | 아두이노 게임기 × MakeCode Arcade | 휴대용 게임기 | [MakeCode Arcade](https://arcade.makecode.com/) | 90분 |

🏠 **DORO DIMC 홈러닝 키트 — QR 접속으로 단계별 학습 가능!**

## 🔗 Links

- [DORO Change X](https://www.changex.org/kr/doro)
- [DORO 홈페이지](https://www.doro-robot.com)

## 📦 기술 스택

- HTML5 + CSS3 + Vanilla JavaScript
- GitHub Pages 호스팅
- 반응형 디자인 (모바일/태블릿/데스크톱)

---

(주)도로 · Do it! Robot
