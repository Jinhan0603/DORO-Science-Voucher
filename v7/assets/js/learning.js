...
(function () {
  ...
  var CERTIFICATE_PRINT_CLASS = 'print-certificate-mode';
  var certificatePrintCleanupTimer = null;
  ...
  function withPixelIcon(name, text, className, position) {
    ...
    var safeText = '<span class="pixel-icon-text">' + escapeHtml(text == null ? '' : text) + '</span>';
    return position === 'after' ? safeText + icon : icon + safeText;
  }

  function clearCertificatePrintMode() {
    if (certificatePrintCleanupTimer) {
      window.clearTimeout(certificatePrintCleanupTimer);
      certificatePrintCleanupTimer = null;
    }
    document.body.classList.remove(CERTIFICATE_PRINT_CLASS);
  }
  ...
  function printCertificateSection() {
    ...
    clearCertificatePrintMode();
    document.body.classList.add(CERTIFICATE_PRINT_CLASS);
    ...
    window.requestAnimationFrame(function () {
      window.requestAnimationFrame(function () {
        try {
          ...
          certificatePrintCleanupTimer = window.setTimeout(clearCertificatePrintMode, 600);
        }
      });
    });
  }
  ...
  function supportContactHtml(messageKo, messageEn, extraClass) {
    ...
    return '' +
      '<div class="' + className + '">' +
        ...
        '<div class="support-email-line">' + withPixelIcon('email', SUPPORT_EMAIL, 'pixel-icon-meta') + '</div>' +
        '<div class="support-contact-actions">' +
          ...
          '<button type="button" class="support-link support-link-email support-email-copy" data-copy-email="' + SUPPORT_EMAIL + '" aria-label="' + escapeHtml(t('learning.support.email.copy', '이메일 복사', 'Copy Email')) + '">' + withPixelIcon('email', t('learning.support.email.copy', '이메일 복사', 'Copy Email'), 'pixel-icon-button') + '</button>' +
        '</div>' +
  ...
  function handleSupportEmailClick(event) {
    ...
    copyTextToClipboard(button.getAttribute('data-copy-email') || SUPPORT_EMAIL)
      .then(function () {
        updateSupportFeedback(button, t('learning.support.email.copied', '이메일 주소를 복사했어요.', 'Email copied.'));
      })
      .catch(function () {
        updateSupportFeedback(button, t('learning.support.email.fallback', '이메일 주소를 직접 복사해요.', 'Please copy the email address manually.'));
      });
  }
  ...
  function renderSafetyQuest(kitId, kit, progress) {
    ...
    var remainingCount = Math.max(promises.length - checkedCount, 0);
    var progressText = t('learning.safety.progress', '안전 약속 진행', 'Safety progress') + ' <strong>' + checkedCount + ' / ' + promises.length + '</strong>';
    var guideTitle = saved.completed
      ? t('learning.safety.complete.title', '안전 약속이 완료되었어요.', 'Safety promise completed.')
      : t('learning.safety.guide.title', '안전 약속을 모두 확인하면 만들기를 시작할 수 있어요.', 'Check every safety promise to start building.');
    var guideCopy = saved.completed
      ? t('learning.safety.complete.copy', '이제 구성품 확인을 시작할 수 있어요.', 'You can now start checking components.')
      : (remainingCount > 0
        ? t('learning.safety.guide.waiting', '카드를 눌러 안전 약속을 모두 확인해요. 모두 확인한 뒤, 아래의 구성품 확인 시작 버튼을 눌러요.', 'Tap each safety card to confirm it. After checking every promise, use the button below to start the component check.')
        : t('learning.safety.guide.copy', '아래 카드를 눌러 안전 약속을 확인해요.', 'Tap the cards below to confirm the safety promise.'));
    var actionCopy = saved.completed
      ? t('learning.safety.action.ready', '안전 약속 확인이 끝났어요. 아래 버튼을 눌러 구성품 확인을 시작해요.', 'Safety promise complete. Use the button below to start the component check.')
      : t('learning.safety.action.waiting', '아직 확인하지 않은 안전 약속이 있어요. 카드를 눌러 모두 확인하면 아래 버튼이 밝게 바뀌어요.', 'Some safety promises are still unchecked. Tap every card and the button below will light up.');
    var actionLabel = saved.completed
      ? t('learning.safety.component.cta', '구성품 확인으로 이동', 'Go to Component Check')
      : t('learning.safety.component.cta.pending', '모두 확인하면 구성품 확인 시작', 'Check all to start Component Check');

    sec.innerHTML =
      '<h2 class="student-section-title">' + buildSectionTitle('learning.safety.title', '안전 약속', 'Safety Promise') + '</h2>' +
      '<p class="student-section-desc">' + t('learning.safety.desc', '아래 약속을 모두 확인한 뒤, 구성품 확인으로 넘어가요.', 'Check every promise below, then move on to the component check.') + '</p>' +
      '<div class="learning-guide-box safety-guide-box' + (saved.completed ? ' is-complete' : ' is-warning') + '">' +
        '<div class="learning-guide-copy">' +
          '<strong>' + guideTitle + '</strong>' +
          '<p>' + guideCopy + '</p>' +
        '</div>' +
        '<div class="learning-guide-meta">' +
          '<span class="safety-progress-chip">' + progressText + '</span>' +
          '<span class="learning-guide-progress">' + (promises.length ? Math.round((checkedCount / promises.length) * 100) : 0) + '%</span>' +
        '</div>' +
      '</div>' +
      '<div class="safety-card">' +
        '<div class="safety-card-head">' +
          ...
          (saved.completed
            ...
            : '') +
        '</div>' +
        '<div class="safety-intro-copy">' + t('learning.safety.copy', '안전 약속 카드를 눌러 하나씩 확인해요. 모두 확인해야 다음 단계로 넘어갈 수 있어요.', 'Tap each safety card to check it. You can move on only after every promise is confirmed.') + '</div>' +
        '<div class="safety-checklist">' +
          promises.map(function (item) {
            var checked = saved.checkedIds.indexOf(item.id) !== -1;
            return '' +
              '<label class="promise-item' + (checked ? ' is-checked' : '') + '" data-promise-id="' + item.id + '">' +
                ...
                '</span>' +
                '<span class="promise-state-badge' + (checked ? ' is-checked' : '') + '">' +
                  withPixelIcon(checked ? 'check' : 'warning', checked
                    ? t('learning.safety.card.checked', '확인 완료', 'Checked')
                    : t('learning.safety.card.pending', '눌러서 확인하기', 'Tap to check'), 'pixel-icon-inline') +
                '</span>' +
              '</label>';
          }).join('') +
        '</div>' +
        '<div class="step-action-panel safety-action-panel' + (saved.completed ? ' is-ready' : ' is-warning') + '">' +
          '<div class="step-action-copy">' +
            '<strong>' + (saved.completed
              ? t('learning.safety.complete', '안전 약속 완료', 'Safety Complete')
              : t('learning.safety.waiting.title', '모든 안전 약속을 확인해요.', 'Check every safety promise.')) + '</strong>' +
            '<p>' + actionCopy + '</p>' +
          '</div>' +
          '<div class="safety-complete-actions">' +
            '<button type="button" class="resource-link-btn safety-component-btn' + (saved.completed ? ' is-ready-to-advance' : ' is-awaiting-action') + '" aria-disabled="' + (saved.completed ? 'false' : 'true') + '">' + withPixelIcon(saved.completed ? 'arrow-next' : 'lock', actionLabel, 'pixel-icon-button') + '</button>' +
    ...
    sec.onclick = function (event) {
      var promiseCard = event.target.closest('.promise-item');
      if (promiseCard) {
        var nextCheckedIds = toggleSafetyChecked(saved.checkedIds, promiseCard.getAttribute('data-promise-id'));
      ...
      if (event.target.closest('.safety-component-btn')) {
        if (!saved.completed) {
          showLearningToast(t('learning.safety.lock.toast', '아직 확인하지 않은 안전 약속이 있어요. 카드를 눌러 모두 확인해 주세요.', 'Some safety promises are still unchecked. Tap every card to confirm them first.'));
          return;
        }
  ...
  function renderCertificate(kitId, kit, progress) {
    ...
    sec.innerHTML =
      '<h2 class="student-section-title">' + buildSectionTitle('learning.certificate.title', '인증서', 'Certificate') + '</h2>' +
      '<div class="certificate-card" id="certificate-card">' +
        ...
        '</div>' +
        '<div class="cert-footer">DOROLAND V7 · DORO DIMC</div>' +
      '</div>' +
      '<div class="cert-btns">' +
        '<button type="button" class="cert-print-btn" data-cert-action="print" aria-label="' + escapeHtml(t('learning.certificate.print', '인증서 출력하기', 'Print certificate')) + '">' + withPixelIcon('print', t('learning.certificate.print', '인증서 출력하기', 'Print certificate'), 'pixel-icon-button') + '</button>' +
        '<button type="button" class="cert-reset-btn" data-cert-action="reset">' + withPixelIcon('reset', t('learning.progress.reset', '전체 진행 초기화', 'Reset all progress'), 'pixel-icon-button') + '</button>' +
        '<p class="cert-note">' + t('learning.certificate.note', '닉네임을 바꾸려면 위의 기록하기에서 수정한 뒤 페이지를 새로고침해요.', 'To change the nickname, update it above and refresh the page.') + '</p>' +
      '</div>';

    sec.onclick = function (event) {
      if (event.target.closest('.cert-print-btn')) {
        event.preventDefault();
  ...
  function renderComponentCheck(kitId, kit, progress) {
    ...
    var guideTitle = partsComplete
      ? t('learning.parts.guide.complete', '모든 부품 확인이 끝났어요.', 'Every part has been checked.')
      : t('learning.parts.guide.title', '부품을 찾았다면 카드를 눌러 체크해요.', 'Tap the card after you find each part.');
    var guideCopy = partsComplete
      ? (missingCount > 0
        ? t('learning.parts.guide.missing', '없어요로 표시한 부품이 있어요. 보호자와 함께 문의 방법을 확인한 뒤, 아래의 다음 단계 버튼을 눌러요.', 'Some parts are marked missing. Review the support note with a parent or guardian, then use the next-step button below.')
        : t('learning.parts.guide.ready', '모든 항목을 체크했어요. 아래의 완료하고 다음 단계로 버튼을 눌러요.', 'Every item is checked. Use the complete-and-next button below.'))
      : t('learning.parts.guide.waiting', '역할과 찾는 힌트를 보고, 있어요 또는 없어요 버튼으로 상태를 선택해요. 모든 카드를 선택해야 다음 단계로 갈 수 있어요.', 'Read the role and find hint, then choose Have it or Missing. You need to select every card to move on.');

    sec.innerHTML =
      '<h2 class="student-section-title">' + buildSectionTitle('learning.parts.title', '구성품 확인', 'Component Check') + '</h2>' +
      '<p class="student-section-desc">' + t('learning.parts.desc.short', '상자 안의 부품을 하나씩 찾아요. 이름, 역할, 찾는 힌트를 확인하고 상태를 표시하세요.', 'Find each item in the box. Check its name, role, hint, and mark its status.') + '</p>' +
      '<div class="learning-guide-box parts-guide-box' + (partsComplete ? ' is-complete' : ' is-warning') + '">' +
        '<div class="learning-guide-copy">' +
          '<strong>' + guideTitle + '</strong>' +
          '<p>' + guideCopy + '</p>' +
        '</div>' +
        '<div class="learning-guide-meta">' +
          '<span class="parts-progress-chip">' + t('learning.parts.progress', '확인한 부품', 'Checked parts') + ' <strong>' + checkedCount + ' / ' + total + '</strong></span>' +
          '<span class="learning-guide-progress">' + (total ? Math.round((checkedCount / total) * 100) : 0) + '%</span>' +
        '</div>' +
      '</div>' +
    ...
    kit.parts.forEach(function (part, i) {
      ...
      card.setAttribute('data-part-id', partKey);
      card.innerHTML =
        '<div class="part-card-head">' +
          visual +
          '<div class="part-head-copy">' +
            '<div class="part-name">' + part.name + '</div>' +
            '<span class="part-state-badge' + (found ? ' is-found' : missing ? ' is-missing' : ' is-pending') + '">' +
              withPixelIcon(found ? 'check' : missing ? 'warning' : 'parts', found
                ? t('learning.parts.state.found', '있어요 확인', 'Have it selected')
                : missing
                  ? t('learning.parts.state.missing', '없어요 확인', 'Missing selected')
                  : t('learning.parts.state.pending', '선택 필요', 'Selection needed'), 'pixel-icon-inline') +
            '</span>' +
          '</div>' +
        '</div>' +
        '<div class="part-meta">' +
          ...
          '</div>' +
        '</div>' +
        '<p class="part-card-hint">' + (found
          ? t('learning.parts.card.ready', '확인 완료예요. 필요하면 아래 버튼으로 다시 바꿀 수 있어요.', 'Checked. You can change it again with the buttons below.')
          : missing
            ? t('learning.parts.card.missing', '부품이 없으면 보호자와 함께 문의 안내를 확인해요.', 'If the part is missing, review the support guide with a parent or guardian.')
            : t('learning.parts.card.pending', '부품을 찾았다면 아래 버튼을 눌러 상태를 선택해요.', 'After you find the part, use the buttons below to choose its status.')) + '</p>' +
        '<div class="part-btns">' +
          '<button type="button" class="part-btn found-btn' + (found ? ' active' : '') + '" data-part-id="' + partKey + '" data-state="found" aria-pressed="' + (found ? 'true' : 'false') + '" aria-label="' + part.name + ' ' + t('learning.parts.found', '있어요', 'Have it') + '">' + withPixelIcon('check', t('learning.parts.found', '있어요', 'Have it'), 'pixel-icon-button') + '</button>' +
          '<button type="button" class="part-btn missing-btn' + (missing ? ' active' : '') + '" data-part-id="' + partKey + '" data-state="missing" aria-pressed="' + (missing ? 'true' : 'false') + '" aria-label="' + part.name + ' ' + t('learning.parts.missing', '없어요', 'Missing') + '">' + withPixelIcon('warning', t('learning.parts.missing', '없어요', 'Missing'), 'pixel-icon-button') + '</button>' +
        '</div>' +
        (missing ? '<div class="part-missing-msg">' + supportContactHtml('부품이 없거나 부서졌다면 보호자와 함께 DORO에 문의해 주세요.', 'If a part is missing or broken, contact DORO with a parent or guardian.', 'support-contact--compact') + '</div>' : '');
      grid.appendChild(card);
    });
    ...
    sec.onclick = function (event) {
      ...
      if (card && !event.target.closest('.support-link, .support-email-copy, .part-missing-msg')) {
        var cardId = card.getAttribute('data-part-id');
        var updatedParts = cloneMap(saved);
        updatedParts[cardId] = 'found';
        saveProgress(kitId, {
          parts: updatedParts,
          gates: mergeGateData(kitId, { partsComplete: isPartMapComplete(updatedParts, kit.parts) })
        });
        rerenderLearningPreserveScroll();
        return;
      }
      ...
      if (event.target.closest('.parts-next-btn') || event.target.closest('.parts-build-btn')) {
        if (!partsComplete) {
          showLearningToast(t('learning.gate.parts.waiting', '아직 확인하지 않은 부품이 있어요. 카드를 눌러 모두 선택해 주세요.', 'Some parts are still unchecked. Tap every card to choose its status.'));
          return;
        }
    ...
    if (summary) {
      summary.innerHTML =
        '<div class="step-action-panel parts-action-panel' + (partsComplete ? ' is-ready' : ' is-warning') + '">' +
          '<div class="step-action-copy">' +
            '<strong>' + (partsComplete
              ? t('learning.parts.complete', '구성품 확인이 끝났어요.', 'Component check complete.')
              : t('learning.parts.action.title', '모든 부품 카드를 선택해요.', 'Select every part card.')) + '</strong>' +
            '<p>' + (partsComplete
              ? (missingCount > 0
                ? t('learning.parts.action.missing', '없어요로 표시한 부품이 있어요. 문의 안내를 확인한 뒤, 아래 버튼으로 다음 단계로 넘어가요.', 'Some parts are marked missing. Review the support guide, then use the button below to continue.')
                : t('learning.parts.action.ready', '모든 항목을 체크했어요. 아래의 완료하고 다음 단계로 버튼을 눌러요.', 'Every item is checked. Press the complete-and-next button below.'))
              : t('learning.parts.action.waiting', '아직 선택하지 않은 부품이 있어요. 있어요 또는 없어요 버튼으로 상태를 모두 선택해야 다음 단계로 갈 수 있어요.', 'Some parts are still not selected. Choose Have it or Missing for every card before moving on.')) + '</p>' +
            '<div class="parts-summary-stats">' +
              '<span class="parts-summary-chip">' + t('learning.parts.found', '있어요', 'Have it') + ' <strong>' + foundCount + '</strong></span>' +
              '<span class="parts-summary-chip is-missing">' + t('learning.parts.missing', '없어요', 'Missing') + ' <strong>' + missingCount + '</strong></span>' +
              '<span class="parts-summary-chip is-pending">' + t('learning.parts.state.pending', '선택 필요', 'Selection needed') + ' <strong>' + Math.max(total - checkedCount, 0) + '</strong></span>' +
            '</div>' +
          '</div>' +
          '<button type="button" class="parts-next-btn' + (partsComplete ? ' is-ready-to-advance' : ' is-awaiting-action') + '" aria-disabled="' + (partsComplete ? 'false' : 'true') + '">' + withPixelIcon('arrow-next', t('learning.parts.next', '완료하고 다음 단계로', 'Complete and go to the next step'), 'pixel-icon-button') + '</button>' +
        '</div>';
    }
  }
  ...
  function renderBuildStep(step, idx, total, guide, showStepNotice) {
    ...
    var guideTitle = t('learning.build.guide.title', '현재 단계 ' + (idx + 1) + ' / ' + total, 'Current step ' + (idx + 1) + ' / ' + total);
    var guideCopy = idx === total - 1
      ? t('learning.build.guide.final', '모든 조립이 끝났다면 아래의 완성 확인하기 버튼을 눌러요.', 'If the build is finished, use the Review the finish button below.')
      : t('learning.build.guide.copy', '이 단계를 따라 했다면 아래의 완료하고 다음 단계로 버튼을 눌러요.', 'After you finish this step, press the Finish and go on button below.');
    var actionTitle = idx === total - 1
      ? t('learning.build.action.final.title', '완성 확인 버튼을 눌러 마지막 화면으로 가요.', 'Press the review button to move to the final screen.')
      : t('learning.build.action.next.title', '완료 버튼을 눌러 다음 단계로 넘어가요.', 'Press the finish button to move to the next step.');
    var actionCopy = idx === total - 1
      ? t('learning.build.action.final.copy', '조립이 끝났다면 아래 버튼을 눌러 완성 여부를 확인하고 다음 활동으로 넘어가요.', 'If the build is done, use the button below to confirm the finish and move on.')
      : t('learning.build.action.next.copy', '단계를 마쳤다면 아래 버튼을 반드시 한 번 눌러야 다음 단계가 열려요.', 'When you finish the step, you must press the button below once to unlock the next step.');
    ...
    return '' +
      '<div class="build-step-card">' +
        ...
        '</div>' +
        '<div class="learning-guide-box build-step-guide is-complete">' +
          '<div class="learning-guide-copy">' +
            '<strong>' + guideTitle + '</strong>' +
            '<p>' + guideCopy + '</p>' +
          '</div>' +
          '<div class="learning-guide-meta">' +
            '<span class="build-step-status-chip">' + t('learning.build.step', '단계', 'Step') + ' ' + stepNum + ' / ' + total + '</span>' +
            '<span class="learning-guide-progress">' + progressPct + '%</span>' +
          '</div>' +
        '</div>' +
        '<div class="build-step-title">' + step.title + '</div>' +
        (step.image
        ...
        '<div class="build-step-progress-bar"><div class="build-step-progress-fill" style="width:' + progressPct + '%"></div></div>' +
        stepNoticeHtml +
        '<div class="build-step-cta-panel">' +
          '<div class="step-action-copy">' +
            '<strong>' + actionTitle + '</strong>' +
            '<p>' + actionCopy + '</p>' +
          '</div>' +
          '<button type="button" class="build-btn build-done-btn is-ready-to-advance">' + withPixelIcon('check', doneLabel, 'pixel-icon-button') + '</button>' +
          '<div class="build-step-support-actions">' +
            (idx > 0 ? '<button type="button" class="build-btn build-redo-btn">' + withPixelIcon('arrow-back', t('learning.build.redo', '이전 단계', 'Previous Step'), 'pixel-icon-button') + '</button>' : '') +
  ...
  function renderLearningUI() {
    ...
    addLearningTOC();
    window.dispatchEvent(new CustomEvent('doro:v7learningrendered'));
  }
